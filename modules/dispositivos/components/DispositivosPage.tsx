      ...
      <div className="mx-auto w-full max-w-6xl space-y-5">
        <header className="rounded-2xl border border-white/50 bg-white/80 p-5 shadow-lg backdrop-blur md:p-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h1 className="text-2xl font-semibold tracking-tight text-slate-800 md:text-3xl">Lista de Dispositivos</h1>
              <p className="text-sm text-slate-500">Administra los equipos de marcacion y prueba su conectividad.</p>
            </div>
          </div>
        </header>
                  ...
                  <form className="space-y-4" onSubmit={handleGuardar}>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-4 text-sm text-slate-700">
                        <label className="inline-flex items-center gap-2">
                          <input type="radio" name="direccionTipo" checked={form.direccionTipo === "ip"} onChange={() => setForm((prev) => ({ ...prev, direccionTipo: "ip" }))} />
                          Direccion IP
                        </label>
                        <label className="inline-flex items-center gap-2">
                          <input type="radio" name="direccionTipo" checked={form.direccionTipo === "dominio"} onChange={() => setForm((prev) => ({ ...prev, direccionTipo: "dominio" }))} />
                          Dominio
                        </label>
                      </div>
                      <Input
                    ...
                    <div className="grid gap-3 md:grid-cols-3">
                      <div className="space-y-1">
                        ...
                        <Input value={form.comunicacion} onChange={(e) => setForm((prev) => ({ ...prev, comunicacion: e.target.value }))} />
                      </div>
                      <div className="space-y-1">
                        <label className="text-sm font-semibold text-slate-700">Puerto:</label>
                        <Input value={form.puerto} onChange={(e) => setForm((prev) => ({ ...prev, puerto: e.target.value }))} />
                      </div>
                      <div className="space-y-1">
          ...
          <div className="max-h-[520px] overflow-auto">
            <table className="w-full min-w-[1080px]">
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan={7} className="border-t border-slate-200 px-4 py-4 text-center text-sm text-slate-500">Cargando dispositivos...</td>
                  </tr>
                  ...
                  <tr key={item.id} className={index % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                    ...
                    <td className="border-t border-slate-200 px-4 py-3 text-slate-700">{item.uso === "ASISTENCIA" ? "Control de Asistencia" : "Control de Acceso"}</td>
                ...
                {!loading && dispositivos.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="border-t border-slate-200 px-4 py-4 text-center text-sm text-slate-500">No se encontraron dispositivos.</td>
                  </tr>
                ) : null}
              </tbody>
            </table>
