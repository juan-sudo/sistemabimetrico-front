...
type SucursalPageSkeletonProps = {
  rows?: number
}

export function SucursalPageSkeleton({ rows = 8 }: SucursalPageSkeletonProps) {
  return (
        ...
        <table className="w-full min-w-[980px]">
          ...
          <tbody>
            {Array.from({ length: rows }).map((_, index) => (
              <tr key={`sucursal-skeleton-${index}`} className={index % 2 === 0 ? "bg-white" : "bg-slate-50"}>
