...
import { Download, Pencil, PlusCircle, ShieldCheck, Trash2 } from "lucide-react"
...
export default function Page() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const data = await authRequest(apiEndpoints.tiposSindicato, { token })
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      setSaving(true)
      if (editingId) {
        const updated = (await authRequest(`${apiEndpoints.tiposSindicato}${editingId}/`, {
          method: "PUT",
          body: payload,
          token,
        })) as TipoSindicato
        ...
        const created = (await authRequest(apiEndpoints.tiposSindicato, {
          method: "POST",
          body: payload,
          token,
        })) as TipoSindicato
  ...
  const onDelete = async (item: TipoSindicato) => {
    ...
    try {
      await authRequest(`${apiEndpoints.tiposSindicato}${item.id}/`, { method: "DELETE", token })
    ...
    <section className="min-h-[calc(100vh-7rem)] bg-[radial-gradient(circle_at_top_right,#dcfce7_0%,#f8fafc_45%,#eef2ff_100%)] p-3 md:p-6">
      <div className="mx-auto w-full max-w-6xl space-y-5">
        <header className="rounded-2xl border border-white/50 bg-white/80 p-5 shadow-lg backdrop-blur md:p-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h1 className="text-2xl font-semibold tracking-tight text-slate-800 md:text-3xl">Lista de sindicatos</h1>
                <p className="text-sm text-slate-500">Administra los tipos de sindicato del sistema.</p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                ...
                <DialogContent className="max-w-xl">
                  <DialogHeader>
                    <DialogTitle>{editingId ? "Editar sindicato" : "Nuevo sindicato"}</DialogTitle>
                  </DialogHeader>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <label className="mb-2 block text-sm font-medium text-slate-700">Buscar sindicato</label>
          <Input
            value={search}
            ...
            <table className="w-full min-w-[980px]">
              <thead className="sticky top-0 z-10 bg-teal-700 text-white">
                <tr className="text-sm">
                  ...
                  <th className="w-44 px-4 py-3 text-left font-semibold">Inicio de convenio</th>
                  <th className="w-40 px-4 py-3 text-left font-semibold">Fin de convenio</th>
                  <th className="w-24 px-4 py-3 text-left font-semibold">Estatus</th>
                ...
                {loading ? (
                  ...
                  filteredRows.map((item, index) => (
                    <tr key={item.id} className={index % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                      ...
                      <td className="border-t border-slate-200 px-4 py-3 text-slate-700">-</td>
                      <td className="border-t border-slate-200 px-4 py-3 text-slate-700">-</td>
                      <td className="border-t border-slate-200 px-4 py-3 text-slate-700">{item.activo ? "Activo" : "Inactivo"}</td>
                      <td className="border-t border-slate-200 px-4 py-3 text-center">
                        <button onClick={() => onEdit(item)} className="inline-flex rounded-lg border border-amber-200 bg-amber-50 p-2 text-amber-600 transition hover:bg-amber-100" aria-label="Editar sindicato">
                          <Pencil size={16} />
                        </button>
                      </td>
                      <td className="border-t border-slate-200 px-4 py-3 text-center">
                        <button onClick={() => onDelete(item)} className="inline-flex rounded-lg border border-rose-200 bg-rose-50 p-2 text-rose-600 transition hover:bg-rose-100" aria-label="Eliminar sindicato">
                          <Trash2 size={16} />
                        </button>
                      </td>
                    </tr>
