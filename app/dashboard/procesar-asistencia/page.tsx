...
type Dispositivo = { id: number; activo: boolean; nombre: string }
...
export default function ProcesarAsistenciaPage() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        const [p, e, s, a, d] = await Promise.all([
  ...
  const handleProcesar = async () => {
    ...
    try {
      ...
      const descarga = await authRequest(`${apiEndpoints.descargasMarcaciones}descargar-dispositivo/`, {
        method: "POST",
        body: {
          dispositivo_ids: dispositivos.map((item) => item.id),
