...
export default function Page() {
  ...
  const descargarMarcaciones = async () => {
    ...
    try {
      ...
      const totalSinPersonal = resultados.reduce((acc, item) => {
  ...
  const verRawDispositivo = async () => {
    if (!token || selectedIds.length === 0) return
    try {
      setReadingRaw(true)
      const response = await authRequest(`${apiEndpoints.descargasMarcaciones}ver-raw-dispositivo/`, {
        method: "POST",
        body: {
          dispositivo_ids: selectedIds,
          clave_comunicacion: 0,
          fecha_inicio: fechaInicioRaw || undefined,
          fecha_fin: fechaFinRaw || undefined,
        },
        token,
      })
  ...
  const verCapacidadDispositivo = async () => {
    if (!token || selectedIds.length === 0) return
    try {
      setReadingCapacity(true)
      const response = await authRequest(`${apiEndpoints.descargasMarcaciones}ver-capacidad-dispositivo/`, {
        method: "POST",
        body: { dispositivo_ids: selectedIds, clave_comunicacion: 0 },
        token,
      })
  ...
  const descargarRawPreviewJson = () => {
    ...
    a.href = url
    a.download = `raw-dispositivo-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-")}.json`
    a.click()
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            ...
            onClick={verCapacidadDispositivo}
          >
        ...
        <div className="space-y-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
          ...
          <div className="max-h-72 overflow-auto rounded-md border border-slate-200 bg-white p-2">
            <pre className="text-xs text-slate-700">
              {JSON.stringify(rawPreview.slice(0, 100), null, 2)}
            </pre>
          </div>
          <p className="text-xs text-slate-500">
            Se muestran maximo 100 en pantalla. El JSON descargado incluye todos los registros.
          </p>
        </div>
      ) : null}
              ...
              <tbody>
                {capacityPreview.map((item, idx) => (
                  <tr key={idx} className={idx % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                    ...
                    <td className="border-t border-slate-200 px-3 py-2">
                      {`${String(item.usuarios_usados ?? "-")} / ${String(item.usuarios_capacidad ?? "-")}`}
                    </td>
