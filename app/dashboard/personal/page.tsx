...
export default function Page() {
  ...
  const onSave = async (e: FormEvent) => {
    e.preventDefault()
    if (!token) return
    if (!form.tipo_documento || !form.tipo_trabajador || !form.categoria) {
      toast.error("Selecciona tipo de documento, tipo de trabajador y categoria.")
      return
    }
    const payload = {
            ...
            <Dialog open={open} onOpenChange={setOpen}>
              ...
              <DialogContent className="max-w-4xl">
                ...
                <form className="grid gap-3 md:grid-cols-3" onSubmit={onSave}>
                  ...
                  <select className="h-9 rounded-md border px-3 text-sm" value={form.tipo_documento} onChange={(e) => setForm((p) => ({ ...p, tipo_documento: e.target.value }))}>
                    <option value="" disabled>Tipo de documento</option>
                    {tiposDoc.map((x) => <option key={x.id} value={x.id}>{x.descripcion}</option>)}
                  </select>
